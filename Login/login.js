...
function showPanel(id) {
    ...
    if (alvo) {
        ...
        // FIX: rola o painel de volta ao topo ao trocar de painel
...
// FIX: funciona com input nativo (checkbox nativo usa .checked diretamente)
...
(function iniciarTransicao() {
    ...
    function triggerTransition() {
        ...
        setTimeout(() => appWrapper.classList.add('app-visible'), 100);
    }
...
// ===============================
// AGUARDA main.html ser injetado
// ===============================
...
function iniciarLogin() {
    ...
    const chkLembrar      = document.getElementById('lembrarSenha');   // agora input nativo
    ...
    // FIX: sem depender de md-checkbox (input nativo funciona direto)
    // ===============================
    if (localStorage.getItem(STORAGE_KEY_LEMBRAR) === 'true') {
        ...
        // Aguarda só md-outlined-text-field; md-checkbox não existe mais
        customElements.whenDefined('md-outlined-text-field')
            .then(definirValores)
            .catch(definirValores);
    }
    ...
    // LIMPAR — LOGIN
    // ===============================
    document.getElementById('btnLimparLogin')?.addEventListener('click', () => {
        clearValue(inputUsuario);
        clearValue(inputSenha);
        setChecked(chkLembrar, false);
        clearError(inputUsuario, document.getElementById('erroUsuario'));
        clearError(inputSenha,   document.getElementById('erroSenha'));
        if (alertMsg) alertMsg.className = 'alert-msg';
        inputUsuario?.focus();
    });
    ...
    document.getElementById('btnLimparCadastro')?.addEventListener('click', () => {
        ...
        ['erroCadNome','erroCadUsuario','erroCadEmail','erroCadSenha','erroCadConfSenha'].forEach(id => {
            const el = document.getElementById(id);
            if (el) el.classList.remove('show');
        });
        [cadNome, cadUsuario, cadEmail, cadSenha, cadConfSenha].forEach(el => el?.classList.remove('error-field'));
        if (alertCadastro) alertCadastro.className = 'alert-msg';
        cadNome?.focus();
    });
    ...
    // LIMPAR — RECUPERAR
    // ===============================
    document.getElementById('btnLimparRecuperar')?.addEventListener('click', () => {
        clearValue(recEmail);
        clearError(recEmail, document.getElementById('erroRecEmail'));
        if (alertRecuperar) alertRecuperar.className = 'alert-msg';
        recEmail?.focus();
    });
    ...
    btnLogin?.addEventListener('click', function () {
        ...
        setTimeout(() => {
            ...
            const usuario  = usuarios.find(
                u => u.usuario === getValue(inputUsuario) && u.senha === getRawValue(inputSenha)
            );
            ...
            sessionStorage.setItem(SESSION_USUARIO_LOGADO, usuario.nome || getValue(inputUsuario));
    ...
    btnCadastrar?.addEventListener('click', function () {
        ...
        const nomeVal = getValue(cadNome);
        if (!nomeVal) {
            showError(cadNome, document.getElementById('erroCadNome'), 'Por favor, informe seu nome completo.');
            valido = false;
        } else if (!nomeVal.includes(' ') || nomeVal.split(' ').filter(p => p.length > 0).length < 2) {
            showError(cadNome, document.getElementById('erroCadNome'), 'Informe nome e sobrenome (ex: João Silva).');
            valido = false;
        }
        if (!getValue(cadUsuario))                         { showError(cadUsuario,   document.getElementById('erroCadUsuario'),  'Por favor, informe um nome de usuário.');          valido = false; }
        if (!regexEmail.test(getValue(cadEmail)))          { showError(cadEmail,     document.getElementById('erroCadEmail'),    'Informe um e-mail válido.');                      valido = false; }
        if (getRawValue(cadSenha).length < 6)              { showError(cadSenha,     document.getElementById('erroCadSenha'),    'A senha precisa ter ao menos 6 caracteres.');     valido = false; }
        if (getRawValue(cadConfSenha) !== getRawValue(cadSenha)) { showError(cadConfSenha, document.getElementById('erroCadConfSenha'), 'As senhas não coincidem.');              valido = false; }
